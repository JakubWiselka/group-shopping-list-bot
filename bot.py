...
logging.basicConfig(filename=f'{os.path.dirname(os.path.abspath(__file__))}/logs.log', level=logging.WARNING)
...
def _create_new_table(chat_id):
    ...
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    c.execute(f"""CREATE TABLE {table} (id INTEGER PRIMARY KEY AUTOINCREMENT, product TEXT)""")
    conn.commit()
    conn.close()
...
def welcome_message(msg):
    bot.send_message(msg.chat.id, f"Hi {msg.from_user.first_name.title()} 👋\n" + WELCOME, parse_mode="html")
...
def command_delete_handler(call):
    if call.data == "no":
        ...
        conn.commit()
        bot.edit_message_text(chat_id=call.message.chat.id,text=CLEAR ,message_id=call.message.message_id)
...
def command_list(msg, edit_mode=False):
    ...
    if edit_mode:
        bot.send_message(msg.chat.id, text, reply_markup=makeKeyboard(list), parse_mode="html")
    else:
...
def command_add(msg):
    ## add exeption when table not exist
    if not _check_if_table_exist(msg.chat.id):
        _activate_bot(msg)
        return
    try:
        text = msg.text[5:]
        products = ",".join('("' + p.strip().lower() + '")' for p in text.split(',') if p)
        if not products:
            return
        table = _get_table_name(msg.chat.id)
        conn = sqlite3.connect(DB)
...
def handle_query(call):
    ...
    try:
        ...
        id = int(id)
        if action == "add":
            c.execute(f"INSERT INTO {table} (product) VALUES ('{list[id][1]}');")
            conn.commit()
        elif action == "rm":
            c.execute(f"DELETE FROM {table} WHERE product = '{list[id][1]}' LIMIT 1;")
            conn.commit()
        elif action == "del":
        ...
        bot.edit_message_text(
            ...
            reply_markup=None if not list or action == "clo" else makeKeyboard(list),
            parse_mode='HTML'
        )
    except Exception as e:
        logging.warning(e)
...
#     previous = c.fetchone()
#     if previous[0] + 1 == next_number and previous[1] != msg.from_user.id:
#         c.execute(f"INSERT INTO {table} (author_id) VALUES ({msg.from_user.id})")
#         conn.commit()
#     else:
#         bot.delete_message(msg.chat.id, msg.message_id)
#     c = conn.close()
